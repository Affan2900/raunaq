...
class _PhotographyScreenState extends State<PhotographyScreen> {
  ...
  void _showAddPhotographyDialog(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Add New Photographer'),
          ...
          child: Column(
            ...
            children: [
              TextField(
                ...
                decoration: const InputDecoration(labelText: 'Name / Studio'),
              ),
              TextField(
                controller: typeController,
                decoration: const InputDecoration(
                  labelText: 'Service (e.g. Wedding, Event)',
                ),
              ),
              TextField(
                controller: priceController,
                decoration: const InputDecoration(
                  labelText: 'Package Price (e.g. PKR 75k)',
                ),
              ),
            ],
          ),
  ...
  void _showEditPhotographyDialog(
  ...
  void _deletePhotography(String docId) async {
    await _firestore
        .collection('categories')
        .doc('photography')
        .collection('items')
      ...
      appBar: AppBar(
        ...
        actions: [
          if (adminMode && _currentUserEmail != null)
            IconButton(
              onPressed: () => _showAddPhotographyDialog(context),
                    ...
                    return GestureDetector(
                      onTap: () {
                        if (isOwner && adminMode) {
                          _showEditPhotographyDialog(context, doc.id, data);
                        } else if (!isOwner) {
                        ...
                        children: [
                          ...
                          if (isOwner && adminMode)
                            Positioned(
                              ...
                              right: 8,
                              child: IconButton(
                                onPressed: () => _deletePhotography(doc.id),
                                icon: const Icon(
